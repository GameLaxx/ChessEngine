...
class ChessParams():
    ...
    def random(self) -> 'ChessParams':
        """
        Returns:
            (ChessParams) : Create a random set of parameters.
        """
...
class ChessBot():
    ...
    def __rule_material(self) -> float:
        """
        Returns:
            (float) : The difference of material in the position.
        """
        ret = 0
    ...
    def __rule_pawnspace(self) -> float:
        """
        Returns:
            (float) : The difference of space behind the pawns.
        """
        white_pawn_squares = self.current_board.bitboard_to_squares(self.current_board.bitboards[self.WHITE][0])
        black_pawn_squares = self.current_board.bitboard_to_squares(self.current_board.bitboards[self.BLACK][0])
        white_space = 0
        black_space = 0
    ...
    def __rule_pawncenter(self) -> float:
        """
        Returns:
            (float) : The difference between the number of pawns in the center.
        """
        center_white_bb = self.current_board.bitboards[self.WHITE][0] & self.center_squares
        center_black_bb = self.current_board.bitboards[self.BLACK][0] & self.center_squares
    ...
    def __rule_squareattacked(self) -> float:
        """
        Returns:
            (float) : The difference between the number of square attacked.
        """
    ...
    def __rule_kingsafety(self) -> float:
        """
        Returns:
            (float) : The difference between the number of square attacked around the king.
        """
        white_attacks = self.current_board.board_attacked(self.BLACK)
        black_attacks = self.current_board.board_attacked(self.WHITE)
        count = 0
        for color in [self.WHITE,self.BLACK]:
            king_index = self.current_board.bitboards[color][self.current_board.KING].bit_length() - 1
    ...
    def __rule_bishoppair(self) -> float:
        """
        Returns:
            (float) : Rewards the side with the bishop pair.
        """
    ...
    def evaluate(self) -> float:
        """
        Returns:
            (float) : For a position, go through all the responses of the opponent to give the score for the best response.
        """
        ret = None
        for move in self.current_board.get_moves(self.current_board.player_turn): # same behaviour as make_decision method
    ...
    def make_decision(self, board : ChessGame) -> str:
        """
        Returns:
            (str) : Go through all the moves and assign a score for each. Then send the best move to play.
        """
        self.current_board = board # store the board to avoid passing it to every function
        moves = self.current_board.get_moves(self.current_board.player_turn) # get all the moves for the current position
        max_score = None
        to_play = [] # we randomly chose the best move from a list of same score moves
        for move in moves:
            self.current_board._move(move) # push the current position before moving, pop to return to position and player
            player_attacked = (self.current_board.player_turn + 1) % 2 # manually change player
            self.current_board.player_turn = player_attacked
            score = self.evaluate()
            if max_score == None: # always store the score if none was added before
                max_score = score
                to_play.append(move)
                self.current_board._pop()
                continue
            if score == max_score: # if same score then add to the list
                to_play.append(move)
                self.current_board._pop()
                continue
            if player_attacked == self.BLACK: # if black then add only if the score is worse
                if max_score < score:
                    max_score = score
                    to_play = [move]
                self.current_board._pop()
                continue
            if max_score > score: # if white the add only if the score is better
                max_score = score
                to_play = [move]
            self.current_board._pop()
            continue
